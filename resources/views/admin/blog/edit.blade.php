...
<style>
    ...
    * { box-sizing: border-box; }
    ...
    /* Meta info bar */
    .meta-bar { display:flex; align-items:center; gap:1.5rem; padding:.75rem 1.6rem; background:var(--bg-surface-2); border-bottom:1px solid var(--border); flex-wrap:wrap; }
    .meta-item { display:flex; align-items:center; gap:6px; font-size:.72rem; color:var(--text-secondary); font-family:'Inter',sans-serif; }
    ...
    .img-current { display:flex; align-items:center; gap:1rem; padding:.9rem; background:var(--bg-surface-2); border:1px solid var(--border); border-radius:var(--r-xs); margin-bottom:.9rem; }
    .img-current img { width:64px; height:64px; object-fit:cover; border-radius:var(--r-xs); border:1px solid var(--border); flex-shrink:0; }
    .img-current .ic-info p { font-size:.78rem; font-weight:600; margin:0 0 2px; }
    .img-current .ic-info span { font-size:.68rem; color:var(--text-muted); }

    .upload-zone { border:2px dashed var(--border-strong); border-radius:var(--r-xs); padding:1.2rem; text-align:center; cursor:pointer; transition:all .2s; background:var(--bg-surface-2); }
    .upload-zone:hover { border-color:var(--accent); background:var(--accent-soft); }
    .upload-zone .uz-icon { font-size:1.4rem; color:var(--text-muted); margin-bottom:.35rem; display:block; }
    .upload-zone p { font-size:.75rem; color:var(--text-secondary); margin:0; }
    .upload-zone .sub { font-size:.66rem; color:var(--text-muted); margin-top:2px; }
    ...
    .unsaved-badge { display:none; align-items:center; gap:5px; background:#fefce8; color:#a16207; border:1px solid rgba(202,138,4,.2); border-radius:50px; padding:4px 11px; font-size:.68rem; font-weight:700; }
    .unsaved-badge.show { display:inline-flex; }

    @keyframes fadeUp { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)} }
    .fu { animation:fadeUp .35s ease-out both; }

    @media(max-width:768px){ .ps{padding:1rem;} .form-row{grid-template-columns:1fr;} .meta-bar{gap:.75rem;} }
</style>
...
<div class="ps">
    ...
    <nav class="bc fu" style="animation-delay:.05s">
        ...
        <span class="sep">/</span>
        <span class="cur">Edit — {{ Str::limit($blog->title, 40) }}</span>
    </nav>

    <div class="ph fu" style="animation-delay:.08s">
        <div>
            <h1>Edit Artikel</h1>
            <p>Memperbarui konten artikel <strong>#B-{{ $blog->id }}</strong> · Dibuat {{ $blog->created_at->translatedFormat('d M Y') }}</p>
        </div>
        <div style="display:flex;align-items:center;gap:.75rem;">
            <span class="unsaved-badge" id="unsaved-badge"><i class="mdi mdi-pencil-circle-outline"></i> Belum disimpan</span>
    ...
    <form action="{{ route('admin.blog.update', $blog) }}" method="POST" enctype="multipart/form-data" id="edit-form">
        @csrf
        @method('PUT')
        ...
        <div class="card-lux fu" style="animation-delay:.12s">
            ...
            {{-- Meta bar --}}
            <div class="meta-bar">
                <div class="meta-item"><i class="mdi mdi-identifier"></i> UID <strong>#B-{{ $blog->id }}</strong></div>
                <div class="meta-item"><i class="mdi mdi-calendar-plus-outline"></i> Dibuat <strong>{{ $blog->created_at->translatedFormat('d M Y, H:i') }}</strong></div>
                <div class="meta-item"><i class="mdi mdi-calendar-edit-outline"></i> Diperbarui <strong>{{ $blog->updated_at->translatedFormat('d M Y, H:i') }}</strong></div>
            </div>
            <div class="card-body">
                <div class="form-row">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $blog->slug) }}"
                                class="fi" placeholder="slug-artikel">
                            <button type="button" class="ig-btn" id="gen-slug" title="Generate ulang dari judul">
        ...
        <div class="card-lux fu" style="animation-delay:.16s">
            ...
            <div class="card-body" style="padding-bottom:1rem;">
                <textarea id="summernote" name="content" class="summernote">{{ old('content', $blog->content) }}</textarea>
                ...
                <div class="card-body">
                    @if($blog->image)
                        <div class="img-current">
                            <img src="{{ asset($blog->image) }}" alt="Gambar saat ini">
                            <div class="ic-info">
                                <p>Gambar saat ini</p>
                                <span>Unggah file baru untuk mengganti gambar ini</span>
                            </div>
                        </div>
                    @endif
                    <div class="upload-zone" onclick="document.getElementById('image-input').click()">
                        <i class="mdi mdi-cloud-upload-outline uz-icon"></i>
                        <p>{{ $blog->image ? 'Klik untuk mengganti gambar' : 'Klik untuk unggah gambar' }}</p>
                    ...
                    <div id="img-preview-wrap" style="display:none;margin-top:.9rem;text-align:center;">
                        <div style="font-size:.68rem;color:var(--accent);font-weight:600;margin-bottom:4px;"><i class="mdi mdi-eye-check-outline"></i> Pratinjau gambar baru</div>
                        <img id="img-preview" style="max-width:100%;height:110px;object-fit:cover;border-radius:var(--r-xs);border:1.5px solid var(--accent);" alt="Preview baru">
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div style="margin-top:.9rem;padding:.8rem;background:var(--accent-soft);border-radius:var(--r-xs);border:1px solid rgba(59,110,248,.15);">
                        ...
                        <input type="datetime-local" name="published_at"
                            ...
                            class="fi" style="font-size:.78rem;padding:8px 10px;background:#fff;">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-lux fu" style="animation-delay:.24s">
            ...
            <div class="card-body">
                <div class="seo-section">
                    ...
                    <div class="form-row" style="margin-bottom:.75rem;">
                        <div class="form-group">
                            ...
                            <input type="text" name="meta_title" id="meta_title"
                                ...
                                class="fi" placeholder="Judul untuk Google Search...">
                            <div class="char-info">
                                <span style="font-size:.68rem;color:var(--text-muted);">Idealnya 50–60 karakter</span>
                                <span class="char-count" id="mt-count">{{ strlen(old('meta_title', $blog->meta_title ?? '')) }}/60</span>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="fl" for="meta_description"><i class="mdi mdi-text-short"></i> Meta Description</label>
                            <textarea name="meta_description" id="meta_description" class="fi" rows="3"
                                placeholder="Ringkasan singkat untuk hasil pencarian...">{{ old('meta_description', $blog->meta_description) }}</textarea>
                            <div class="char-info">
                    ...
                    <div class="seo-preview">
                        ...
                        <div class="prev-title" id="prev-title">{{ old('meta_title', $blog->meta_title ?: $blog->title) }}</div>
                        <div class="prev-url">https://situsanda.com/blog/<span id="prev-slug">{{ old('slug', $blog->slug) }}</span></div>
                        <div class="prev-desc" id="prev-desc">{{ old('meta_description', $blog->meta_description ?: 'Deskripsi meta akan ditampilkan sebagai cuplikan di hasil pencarian Google...') }}</div>
                    </div>
                </div>
            </div>
            <div class="form-actions">
                ...
                <div style="display:flex;gap:.6rem;align-items:center;flex-wrap:wrap;">
                    <span class="unsaved-badge" id="unsaved-badge-2"><i class="mdi mdi-pencil-circle-outline"></i> Ada perubahan belum disimpan</span>
                    <button type="submit" class="btn-p"><i class="mdi mdi-content-save-outline"></i> Simpan Perubahan</button>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function markDirty() {
        if (!isDirty) {
            isDirty = true;
            document.getElementById('unsaved-badge').classList.add('show');
            document.getElementById('unsaved-badge-2').classList.add('show');
        }
    }

    // Track changes on all inputs
    document.getElementById('edit-form').querySelectorAll('input,textarea,select').forEach(el => {
    ...
    function charCount(id, countId, max, prevId) {
        ...
        c.textContent = initLen + '/' + max;
        c.className = 'char-count ' + (initLen > max ? 'warn' : initLen >= max * .8 ? 'ok' : '');
        ...
        el.addEventListener('input', function () {
            ...
            if (prevId) {
                ...
                prev.textContent = this.value || prev.dataset.default;
            }
        });
    }
    document.getElementById('prev-title').dataset.default = '{{ $blog->title }}';
